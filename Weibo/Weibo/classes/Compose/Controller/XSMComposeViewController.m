...
@interface XSMComposeViewController ()

@end

@implementation XSMComposeViewController
...
- (void)setupTextView {
    XSMEmotionTextView *textView = [[XSMEmotionTextView alloc] init];
    ...
    XSMEmotionKeyboard *emotionKeyboard = [[XSMEmotionKeyboard alloc] init];
    ...
    //通知表情按钮被选
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(emotionDidSelect:) name:XSMEmotionButtonDidSelectNotification object:nil];
    
    //通知，删除按钮点击
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(delBtnDidClick:) name:XSMDeleteButtonDidSelectNotification object:nil];
}

- (void)setupToolbar {
    XSMComposeToolbar *toolbar = [[XSMComposeToolbar alloc] init];
...
- (void)setupPhoto {
    XSMComposePhoto *photo = [[XSMComposePhoto alloc] init];
    self.photo = photo;
...
- (void)emotionDidSelect:(NSNotification *)notification {
    XSMEmotion *emotion = notification.userInfo[XSMSelectedEmotionButtonKey];
...
#pragma mark - XSMComposeToolbarDelegate代理方法
- (void)composeToolbar:(XSMComposeToolbar *)composeToolbar didClickButton:(XSMComposeToolbarButtonType)buttonType {
    switch (buttonType) {
        case XSMComposeToolbarButtonTypePicture:
            [self openAlbum];
            break;
        case XSMComposeToolbarButtonTypeCamera: {
            NSLog(@"camera click");
            [self openCamera];
            break;
        case XSMComposeToolbarButtonTypeEmotion:
            [self switchKeyboard];
            break;
        }

...
@implementation XSMStatusFrame

- (void)setStatus:(XSMStatus *)status {
    _status = status;
    ...
    CGSize userNameSize = [self sizeWithText:_status.user.name font:XSMNameFont maxW:MAXFLOAT];
    ...
    CGSize textSize = [self sizeWithText:_status.text font:XSMTextFont maxW:355];
    ...
    if(_status.retweeted_status) {
        XSMStatus *retweetedStatus = _status.retweeted_status;
        NSString *content = [NSString stringWithFormat:@"%@ : %@", retweetedStatus.user.name, retweetedStatus.text];
        CGSize contentSize = [self sizeWithText:content font:XSMTextFont maxW:355];
